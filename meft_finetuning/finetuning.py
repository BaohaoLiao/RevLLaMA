...
def get_args_parser():
    parser = argparse.ArgumentParser("MEFT Fine-tuning", add_help=False)
    parser.add_argument(
        "--batch_size",
        default=64,
        type=int,
    ...
    parser.add_argument(
        "--reversible_layer", type=int, default=0, metavar="MEFT", help="the number of reversible layer"
    )
...
def main(args):
    ...
    # define the model
    model = models_revllama.__dict__[args.model](args)
    model.to(device)
